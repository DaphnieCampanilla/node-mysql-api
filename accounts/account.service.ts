...
async function register(params: any, origin: any) {
    ...
    if (await db.Account.findOne({ where: { email: params.email } })) {
        // send already registered error email to prevent account enumeration
    ...
    // first account registered is an admin
...
async function forgotPassword({ email }: any, origin: any) {
    ...
    // always return ok response to prevent account enumeration
    if (!account) return;
...
async function sendAlreadyRegisteredEmail(email: any, origin: any) {
    let message;
    if (origin) {
        message = `<p>If you don't know your password please visit the <a href="${origin}/account/forgot-password">forgot password</a> 
        page.</p>`;
    } else {
